...
from vendor.models import vendor_master, vendor_daily_PL
...
# username toh address yaa order_id waali foreign ki se le lenge
    username = models.CharField(max_length=120)
    order_id = models.ForeignKey(Order, null = True)
    address = models.ForeignKey(address)
    Loma_master = models.ForeignKey(Loma_master)
    #dp_id = models.ForeignKey(dp)
...
class loma_attendance_model(models.Model):
    loma_master = models.ForeignKey(Loma_master)
    loma_attendance = models.CharField(max_length=120, choices = STATUS_CHOICES, default='A')
    today_date = models.DateField(default=datetime.date.today)
    timestamp = models.DateTimeField(auto_now=False, auto_now_add=True)
    active = models.BooleanField(default = False)

    def __unicode__(self):
        return self.loma_master
